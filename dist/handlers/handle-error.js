...
const handleError = async (overallProblem, errorHandler, defaultErrorResponse, req, res, error) => {
   ...
   */
  const simplifiedError = (0, _extractError.extractError)(error);
  ...
  try {
    ...
    if (overriddenResponse != null) {
      ...
      logger.error(`${overallProblem}; custom error response generated`, {
        ...simplifiedError,
        requestURL,
        kind: _wonderStuffServer.Errors.TransientService
      });
      res.header(headers);
      res.send(body);
      return;
     ...
     */
    const innerError = (0, _extractError.extractError)(customHandlerError);
