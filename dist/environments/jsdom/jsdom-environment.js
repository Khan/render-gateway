  ...
  constructor(configuration) {
    ...
    _defineProperty(this, "_retrieveTargetFiles", async (url, renderAPI, resourceLoader) => {
      ...
      try {
         ...
         */
        const fileURLs = await this._configuration.getFileList(url, renderAPI, url => resourceLoader.fetch(url));
        ...
        return {
          files: await Promise.all(fileURLs.map(f => {
             ...
             */
            if (fetchResult == null) {
              throw new _wonderStuffCore.KindError(`Unable to retrieve ${f}. ResourceLoader returned null.`, _wonderStuffServer.Errors.TransientService);
            }
            /**
    ...
    _defineProperty(this, "render", async (url, renderAPI) => {
      ...
      try {
        ...
        if (typeof vmContext[registrationCallbackName][registeredCbName] !== "function") {
          throw new _wonderStuffCore.KindError("No render callback was registered.", _wonderStuffServer.Errors.Internal);
        }
    ...
    cb();`);
         ...
         */
        if (result == null || !(0, _safeHasOwnProperty.safeHasOwnProperty)(result, "body") || !(0, _safeHasOwnProperty.safeHasOwnProperty)(result, "status") || !(0, _safeHasOwnProperty.safeHasOwnProperty)(result, "headers")) {
          throw new _wonderStuffCore.KindError(`Malformed render result: ${JSON.stringify(result)}`, _wonderStuffServer.Errors.Internal);
        }
    ...
    if (configuration == null) {
      throw new _wonderStuffCore.KindError("Must provide environment configuration", _wonderStuffServer.Errors.Internal);
    }
    ...
    return new Promise(resolve => {
      ...
      setTimeout(async () => {
        const reportCloseableError = e => {
          ...
          logger.error(`Closeable encountered an error: ${simplifiedError.error || ""}`, {
            ...simplifiedError,
            kind: _wonderStuffServer.Errors.Internal
          });
        };
        /**
